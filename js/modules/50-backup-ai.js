...
function arrayToCSV(array, headers) {
    ...
    array.forEach(item => {
        const row = headers.map(h => escapeCSV(item[h] !== undefined ? item[h] : ''));
        rows.push(row.join(','));
    });
    return rows.join('\n');
}
...
function exportClientsAsCSV(clients, filename) {
    ...
    const csv = '\uFEFF' + arrayToCSV(clients, headers); // BOM for Excel UTF-8
...
function exportFullAsCSV(data, dateStr) {
    ...
    // Main: clients
    const clientsCSV = '\uFEFF' + arrayToCSV(getClientsData(), 
        ['nome', 'telefone', 'cpf', 'aniversario', 'email', 'observacoes', 'ultimoAtendimento', 'totalAtendimentos']);
    downloadFile(clientsCSV, `nexus-clientes-${dateStr}.csv`, 'text/csv;charset=utf-8');
    ...
    if (data.data.services && data.data.services.length > 0) {
        ...
        const servicesCSV = '\uFEFF' + arrayToCSV(servicesData, ['id', 'nome', 'preco', 'duracao', 'categoria']);
        downloadFile(servicesCSV, `nexus-servicos-${dateStr}.csv`, 'text/csv;charset=utf-8');
    }
    ...
    if (data.data.products && data.data.products.length > 0) {
        ...
        const productsCSV = '\uFEFF' + arrayToCSV(productsData, ['id', 'nome', 'preco', 'estoque', 'categoria']);
        downloadFile(productsCSV, `nexus-produtos-${dateStr}.csv`, 'text/csv;charset=utf-8');
    }
    ...
    if (data.data.transactions && data.data.transactions.length > 0) {
        ...
        const transCSV = '\uFEFF' + arrayToCSV(transData, ['id', 'data', 'tipo', 'valor', 'descricao', 'formaPagamento', 'bandeira']);
        downloadFile(transCSV, `nexus-transacoes-${dateStr}.csv`, 'text/csv;charset=utf-8');
    }
    ...
    if (data.data.appointments && data.data.appointments.length > 0) {
        ...
        const apptsCSV = '\uFEFF' + arrayToCSV(apptsData, ['id', 'data', 'hora', 'cliente', 'telefone', 'servico', 'profissional', 'status']);
        downloadFile(apptsCSV, `nexus-agendamentos-${dateStr}.csv`, 'text/csv;charset=utf-8');
    }
