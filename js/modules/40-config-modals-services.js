...
function saveService(index) {
    ...
    if (!nome) {
        showToast('Nome do serviÃ§o Ã© obrigatÃ³rio', 'error');
        return;
    }
    
    if (preco <= 0) {
        showToast('PreÃ§o deve ser maior que zero', 'error');
        return;
    }
    
    if (duracao <= 0 || duracao > 480) {
        showToast('DuraÃ§Ã£o deve ser entre 1 e 480 minutos', 'error');
        return;
    }
    ...
    renderServices();
    showToast('ServiÃ§o salvo!', 'success');
}
...
function deleteService(index) {
    ...
    saveServicesToStorage();
    renderServices();
    showToast('ServiÃ§o excluÃ­do', 'success');
}
...
function saveProduct(index) {
    ...
    if (!nome) {
        showToast('Nome do produto Ã© obrigatÃ³rio', 'error');
        return;
    }
