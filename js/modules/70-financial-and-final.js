...
function triggerRestoreBackup() {
    ...
    input.onchange = async (e) => {
        ...
        try {
            ...
            if (!backupData.version || (!backupData.data && !backupData.localStorage)) {
                showToast('Arquivo de backup inválido', 'error');
                return;
            }
            
            if (!confirm('Atenção: A recarga desse backup substituirá todos os dados e finanças atuais no NUVEM. Esta ação não pode ser desfeita. Tem certeza?')) {
                return;
            }
            
            showToast('Enviando arquivos para a nuvem. Aguarde...', 'warning');
            
            // 1. Enviar payload de dados estruturados para a nuvem se a URL do banco estiver configurada.
            if (AppState.dbUrl && backupData.data) {
                 const restoreResp = await fetch(AppState.dbUrl, {
                     method: 'POST',
                     headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                     body: new URLSearchParams({ action: "SYSTEM_RESTORE", payload: JSON.stringify({ backup: backupData.data }) })
                 });
                 if (!restoreResp.ok) throw new Error("Erro de conexão na restauração da nuvem.");
                 const restoreRes = await restoreResp.json();
                 if (!restoreRes.ok) throw new Error(restoreRes.err || "Falha na restauração do servidor.");
            }

            // 2. Restaurar Local Storage (inclusive senhas se possuir admins na maquina local)
            ...
            // 3. Forçar Recarga do App Inteiro
            ...
            showToast('Falha crítica na restauração: ' + err.message, 'error');
        }
    };
    input.click();
}
