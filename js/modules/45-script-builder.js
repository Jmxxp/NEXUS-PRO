...
function applyScriptBuilderWeekdayPreset() {
    ...
    ScriptBuilderState.horarios.sabado = { abre: '08:00', fecha: '18:00' };
...
function buildScriptBuilderScript() {
    ...
    // Horários padrão - Seg-Sex 08:00-18:00, Sáb 08:00-12:00, Dom fechado
    // Se o admin configurar novos horários nas Configurações, esses serão usados
    ...
    return `/**
 * =============================================
 * NEXUS - Sistema de Gestao SaaS v5.0
 ...
 * - Horários de funcionamento configuráveis via Configurações
 *
 * ESTRUTURA:
 * 1. Agendadores    - Admin e funcionarios
 ...
 * 10. Configuracoes - Horários e outras configs
 *
 * COMO USAR:
...
// =====================================================
// CONFIGURACOES INICIAIS v5.0
// =====================================================

// Apenas o admin inicial - outros agendadores são criados pelo admin
var ADMIN_INICIAL = ${JSON.stringify({ id: admin.id, nome: admin.nome, senha: admin.senha, isAdmin: true }, null, 2)};

// Permissões padrão para novos agendadores
var DEFAULT_PERMISSIONS = {
  agenda: true, clientes: true, retorno: true,
  servicos: false, produtos: false, caixa: false,
  comissoes: false, estatisticas: true,
  configuracoes: false, backups: false
};

// Todas as permissões para admin
var ADMIN_PERMISSIONS = {
  agenda: true, clientes: true, retorno: true,
  servicos: true, produtos: true, caixa: true,
  comissoes: true, estatisticas: true,
  configuracoes: true, backups: true
};

// Horarios padrão (fallback - o sistema lê da aba Configuracoes)
var HORARIOS_PADRAO = ${JSON.stringify(horariosPadrao, null, 2)};
...
// COLUNAS v5.0
var COLS = {
  PROF: ["id","nome","senha","isAdmin","status","permissoes","agenda_bloqueada","registrar_expediente","hora_inicio","hora_fim","almoco_inicio","almoco_fim","comissao_servicos","comissao_produtos","created_at","updated_at"],
  ...
  CASH: ["id","tipo","cliente_id","descricao","forma_pagamento","bandeira_cartao","taxa_cartao","valor_original","valor_real","data","hora","status","profissional_id","created_at","updated_at"],
  ...
  CONFIG: ["chave","valor","updated_at"]
};

// =====================================================
// BOOT - INICIALIZA PLANILHA v5.0
// =====================================================
...
function boot() {
  ...
  // Inicializar apenas o ADMIN se tabela vazia
  if (sheets.PROF.getLastRow() <= 1) {
    var now = iso();
    var p = ADMIN_INICIAL;
    sheets.PROF.appendRow([
      p.id, p.nome, p.senha, true,
      "ativo", JSON.stringify(ADMIN_PERMISSIONS), false, false,
      "08:00", "18:00", "12:00", "13:00", 0, 0,
      now, now
    ]);
  }
...
function route(action, body) {
  try {
    switch (action) {
      ...
      // ===== LEGACY SERVIÇOS (compatibilidade v3.x) =====
      case "cadastrarServico": return servCreate(body);
      case "atualizarServico": return servUpdate(body);
      case "excluirServico": return servDelete(body);
      case "listarServicos": return servList(body);
      
      // ===== NOVOS NOMES SERVIÇOS (v4.x) =====
      case "criarServico": return servCreate(body);
      
      // ===== LEGACY PRODUTOS (compatibilidade v3.x) =====
      case "cadastrarProduto": return prodCreate(body);
      case "atualizarProduto": return prodUpdate(body);
      case "excluirProduto": return prodDelete(body);
      case "listarProdutos": return prodList(body);
      
      // ===== NOVOS NOMES PRODUTOS (v4.x) =====
      case "criarProduto": return prodCreate(body);
      case "ajustarEstoque": return prodStockAdjust(body);
      
      // ===== LEGACY CAIXA (compatibilidade v3.x) =====
      case "cadastrarCaixa": return cashCreate(body);
      case "atualizarCaixa": return cashUpdate(body);
      case "cancelarCaixa": return cashCancel(body);
      case "listarCaixa": return cashList(body);
      
      // ===== NOVOS NOMES TRANSACOES (v4.x) =====
      case "criarTransacao": return cashCreate(body);
      case "atualizarTransacao": return cashUpdate(body);
      case "excluirTransacao": return cashCancel(body);
      case "listarTransacoes": return cashList(body);
      ...
      // ===== TAXAS DE CARTÃO =====
      case "getCardFees": return getCardFees();
      case "setCardFees": return setCardFees(body);
      ...
      // ===== BACKUP RESTAURACAO =====
      case "SYSTEM_RESTORE": return systemRestore(body);
      
      // ===== AGENDADORES v5.0 (para admin UI) =====
      case "agendadorCreate": return profCreate(body);
      case "agendadorUpdate": return profUpdate(body);
      case "agendadorDelete": return profDelete(body);
      case "agendadorGet": return agendadorGet(body);
      case "agendadorUpdatePassword": return agendadorUpdatePassword(body);
      case "agendadorUpdatePermissions": return profUpdate({ id: body.id, permissoes: body.permissoes });
      case "agendadorUpdateSchedule": return profUpdate(body);
      case "agendadorUpdateCommission": return profUpdate({ id: body.id, comissao_servicos: body.comissao_servicos, comissao_produtos: body.comissao_produtos });
      case "agendadorToggleBlock": return profSetAgendaBlock({ id: body.id, bloqueado: body.bloqueado });
...
function getHorariosGlobais() {
  ...
  try {
    var config = getConfig({ chave: "horarios_sistema" });
...
function auth(body) {
  ...
  if (!prof) return { ok: false, err: "Agendador nao encontrado" };
  ...
  return {
    ...
    agendador: {
      ...
      agenda_bloqueada: prof.agenda_bloqueada,
      hora_inicio: prof.hora_inicio,
      hora_fim: prof.hora_fim,
      almoco_inicio: prof.almoco_inicio,
      almoco_fim: prof.almoco_fim,
      comissao_servicos: prof.comissao_servicos,
      comissao_produtos: prof.comissao_produtos
    }
  };
}
...
function parsePermissions(val) {
  if (!val) return DEFAULT_PERMISSIONS;
  if (typeof val === "object") return val;
  try { return JSON.parse(val); } catch(e) { return DEFAULT_PERMISSIONS; }
}

function formatProf(r) {
  var perms = parsePermissions(r[5]);
  return {
    id: r[0], nome: str(r[1]), senha: str(r[2]), isAdmin: toBool(r[3]),
    status: str(r[4]) || "ativo",
    permissoes: perms,
    agenda_bloqueada: toBool(r[6]),
    registrar_expediente: toBool(r[7]),
    hora_inicio: str(r[8]), hora_fim: str(r[9]),
    almoco_inicio: str(r[10]), almoco_fim: str(r[11]),
    comissao_servicos: toNum(r[12], 0),
    comissao_produtos: toNum(r[13], 0),
...
function getProfissionaisComConfig(sh) {
  ...
  return rows.map(function(r) {
    ...
    return {
      ...
      agenda_bloqueada: p.agenda_bloqueada,
      hora_inicio: p.hora_inicio, hora_fim: p.hora_fim,
      comissao_servicos: p.comissao_servicos,
      comissao_produtos: p.comissao_produtos
    };
  });
}
...
function profCreate(body) {
  ...
  var now = iso();
  var perms = body.isAdmin ? ADMIN_PERMISSIONS : (body.permissoes || DEFAULT_PERMISSIONS);
  sh.appendRow([
    id, body.nome, body.senha || "", toBool(body.isAdmin),
    body.status || "ativo", JSON.stringify(perms),
    toBool(body.agenda_bloqueada), toBool(body.registrar_expediente),
    body.hora_inicio || "08:00", body.hora_fim || "18:00",
    body.almoco_inicio || "", body.almoco_fim || "",
    toNum(body.comissao_servicos, 0), toNum(body.comissao_produtos, 0),
    now, now
  ]);
  return { ok: true, id: id, msg: "Agendador criado" };
}
...
function profUpdate(body) {
  ...
  var newPerms = body.permissoes !== undefined ? JSON.stringify(body.permissoes) : cur[5];
  ...
  sh.getRange(row, 1, 1, COLS.PROF.length).setValues([[
    ...
    newPerms,
    body.agenda_bloqueada !== undefined ? toBool(body.agenda_bloqueada) : cur[6],
    body.registrar_expediente !== undefined ? toBool(body.registrar_expediente) : cur[7],
    body.hora_inicio !== undefined ? body.hora_inicio : cur[8],
    body.hora_fim !== undefined ? body.hora_fim : cur[9],
    body.almoco_inicio !== undefined ? body.almoco_inicio : cur[10],
    body.almoco_fim !== undefined ? body.almoco_fim : cur[11],
    body.comissao_servicos !== undefined ? toNum(body.comissao_servicos, 0) : cur[12],
    body.comissao_produtos !== undefined ? toNum(body.comissao_produtos, 0) : cur[13],
    cur[14], iso()
  ]]);
...
function profDelete(body) {
  ...
  var sheets = boot();
  var prof = getProfById(sheets.PROF, body.id);
  if (prof && prof.isAdmin && prof.id === ADMIN_INICIAL.id) {
    return { ok: false, err: "Nao pode excluir o admin principal" };
  }
...
function agendadorGet(body) {
  if (!body || !body.id) return { ok: false, err: "ID obrigatorio" };
  var sheets = boot();
  var prof = getProfById(sheets.PROF, body.id);
  if (!prof) return { ok: false, err: "Agendador nao encontrado" };
  
  // Retorna no formato consistente com getProfissionaisComConfig
  return { 
    ok: true, 
    agendador: {
      id: prof.id,
...
function agendadorUpdatePassword(body) {
  if (!body || !body.id || !body.novaSenha) return { ok: false, err: "ID e nova senha obrigatorios" };
  var sheets = boot();
  var prof = getProfById(sheets.PROF, body.id);
  if (!prof) return { ok: false, err: "Agendador nao encontrado" };
  
  // Se não for admin alterando, verifica senha atual
  if (!body.isAdmin && body.senhaAtual !== prof.senha) {
    return { ok: false, err: "Senha atual incorreta" };
  }
  
  return profUpdate({ id: body.id, senha: body.novaSenha });
}
...
function profSetConfig(body) {
  if (!body || !body.agendadorId) return { ok: false, err: "agendadorId obrigatorio" };
  var updateData = { id: body.agendadorId };
  
  // Só atualiza campos que foram definidos
  if (body.almocoInicio !== undefined) updateData.almoco_inicio = body.almocoInicio;
  if (body.almocoFim !== undefined) updateData.almoco_fim = body.almocoFim;
  if (body.bloqueado !== undefined) updateData.agenda_bloqueada = body.bloqueado;
  
  return profUpdate(updateData);
}
...
function getConfigSheet() {
  ...
  if (!sh) {
    ...
    sh.getRange(1, 1, 1, COLS.CONFIG.length).setValues([COLS.CONFIG]).setFontWeight("bold");
    sh.setFrozenRows(1);
  }
  return sh;
}
...
function getConfig(body) {
  if (!body || !body.chave) return { ok: false, err: "Chave obrigatória" };
  var sh = getConfigSheet();
  var data = sh.getDataRange().getValues();
  
  for (var i = 1; i < data.length; i++) {
    if (data[i][0] === body.chave) {
      var valor = data[i][1];
...
function setConfig(body) {
  ...
  var valorStr = typeof body.valor === "object" ? JSON.stringify(body.valor) : body.valor;
  
  if (rowNum === -1) {
    sh.appendRow([body.chave, valorStr, iso()]);
  } else {
    sh.getRange(rowNum, 2).setValue(valorStr);
    sh.getRange(rowNum, 3).setValue(iso());
  }
...
function getCardFees() {
  ...
  var stored = props.getProperty("cardFees");
  if (stored) {
    return { ok: true, fees: JSON.parse(stored) };
  }
  // Valores zerados quando não configurado
  return { ok: true, fees: {
...
function setCardFees(body) {
  if (!body || !body.fees) return { ok: false, err: "Dados de taxas obrigatorios" };
  var props = PropertiesService.getScriptProperties();
  props.setProperty("cardFees", JSON.stringify(body.fees));
  return { ok: true, msg: "Taxas salvas" };
}
...
function formatServ(r) {
  var stat = str(r[6]) || "ativo";
  return {
    id: r[0], 
    nome: str(r[1]), 
    preco: toNum(r[2]), 
    duracao: toNum(r[3]),          // duracao_minutos -> duracao 
    duracao_minutos: toNum(r[3]),  // mantido para compatibilidade
    descricao: str(r[4]), 
    icon: str(r[5]) || "fa-scissors", 
    status: stat,
    ativo: stat === "ativo",       // adiciona ativo para frontend
    created_at: str(r[7]), 
    updated_at: str(r[8])
  };
}
...
function servCreate(body) {
  ...
  var id = body.id || genId();
  var now = iso();
  // Aceita duracao ou duracao_minutos
  var dur = body.duracao_minutos || body.duracao || 30;
  sheets.SERV.appendRow([
    id, body.nome, toNum(body.preco), toNum(dur),
...
function servUpdate(body) {
  ...
  // Aceita duracao ou duracao_minutos
  var novaDur = body.duracao_minutos !== undefined ? body.duracao_minutos : (body.duracao !== undefined ? body.duracao : undefined);
  ...
  sheets.SERV.getRange(row, 1, 1, COLS.SERV.length).setValues([[
    ...
    novaDur !== undefined ? toNum(novaDur) : cur[3],
...
function prodStockAdjust(body) {
  if (!body || !body.id) return { ok: false, err: "ID obrigatorio" };
  var qtd = toNum(body.quantidade, 0);
  if (qtd > 0) return prodStockAdd({ id: body.id, quantidade: qtd });
  if (qtd < 0) return prodStockRemove({ id: body.id, quantidade: Math.abs(qtd) });
  return { ok: true, msg: "Nenhum ajuste (quantidade = 0)" };
}
...
function apptCreateLegacy(body) {
  // Tentar encontrar servicoId pelo nome se não vier o ID
  var servicoId = body.servicoId || null;
  if (!servicoId && body.servico) {
    var sheets = boot();
    var rows = getRows(sheets.SERV, COLS.SERV);
    for (var i = 0; i < rows.length; i++) {
      if (str(rows[i][1]).toLowerCase() === body.servico.toLowerCase()) {
        servicoId = rows[i][0];
        break;
      }
    }
...
function formatCash(r) {
  return {
    id: r[0], 
    type: str(r[1]),           // tipo -> type
    cliente_id: r[2], 
    clientPhone: str(r[2]),    // para compatibilidade frontend
    description: str(r[3]),    // descricao -> description
    paymentMethod: str(r[4]),  // forma_pagamento -> paymentMethod
    cardBrand: str(r[5]),      // bandeira_cartao -> cardBrand
    taxa_cartao: toNum(r[6]), 
    valor_original: toNum(r[7]), 
    value: toNum(r[8]),        // valor_real -> value
    date: fmtDate(r[9]),       // data -> date
    hora: fmtTime(r[10]), 
    status: str(r[11]) || "ativo",
    profissional_id: str(r[12]),
    createdAt: str(r[13]),     // created_at -> createdAt
    lastEditedAt: str(r[14]),  // updated_at -> lastEditedAt
    items: [],
    editHistory: []
  };
}
...
function cashCreate(body) {
  // Mapeia type (inglês) para tipo (português) se necessário
  var tipo = body.tipo || (body.type === 'income' ? 'entrada' : (body.type === 'expense' ? 'saida' : body.type));
  if (!body || !tipo) return { ok: false, err: "Tipo obrigatorio (entrada/saida)" };
  
  var sheets = boot();
  var id = body.id || genId();
  var now = iso();
  // Aceita date ou data
  var data = body.data || body.date || fmtDate(new Date());
  var hora = body.hora || fmtTime(new Date());
  // Aceita campos em inglês ou português
  var descricao = body.descricao || body.description || "";
  var formaPag = body.forma_pagamento || body.paymentMethod || "";
  var bandeira = body.bandeira_cartao || body.cardBrand || "";
  var valor = toNum(body.valor_original || body.value);
  var valorReal = toNum(body.valor_real || body.value);
  var clienteId = body.cliente_id || body.clientPhone || "";
  var profissionalId = body.profissional_id || "";
  
  sheets.CASH.appendRow([
    id, tipo, clienteId, descricao,
    formaPag, bandeira,
    toNum(body.taxa_cartao), valor, valorReal,
    data, hora, body.status || "ativo",
    profissionalId, now, now
  ]);
...
function cashUpdate(body) {
  ...
  // Mapeia campos em inglês para português
  var tipo = body.tipo || (body.type === 'income' ? 'entrada' : (body.type === 'expense' ? 'saida' : body.type));
  var descricao = body.descricao !== undefined ? body.descricao : body.description;
  var formaPag = body.forma_pagamento !== undefined ? body.forma_pagamento : body.paymentMethod;
  var bandeira = body.bandeira_cartao !== undefined ? body.bandeira_cartao : body.cardBrand;
  var valorOrig = body.valor_original !== undefined ? body.valor_original : body.value;
  var valorReal = body.valor_real !== undefined ? body.valor_real : body.value;
  var data = body.data !== undefined ? body.data : body.date;
  var clienteId = body.cliente_id !== undefined ? body.cliente_id : body.clientPhone;
  
  var cur = sheets.CASH.getRange(row, 1, 1, COLS.CASH.length).getValues()[0];
  var profissionalId = body.profissional_id !== undefined ? body.profissional_id : cur[12];
  ...
  sheets.CASH.getRange(row, 1, 1, COLS.CASH.length).setValues([[
    body.id,
    tipo !== undefined ? tipo : cur[1],
    clienteId !== undefined ? clienteId : cur[2],
    descricao !== undefined ? descricao : cur[3],
    formaPag !== undefined ? formaPag : cur[4],
    bandeira !== undefined ? bandeira : cur[5],
    body.taxa_cartao !== undefined ? toNum(body.taxa_cartao) : cur[6],
    valorOrig !== undefined ? toNum(valorOrig) : cur[7],
    valorReal !== undefined ? toNum(valorReal) : cur[8],
    data !== undefined ? data : cur[9],
  ...
  if (body.itens !== undefined) {
    var oldItemRows = getRows(sheets.CASH_ITEMS, COLS.CASH_ITEMS);
    for (var i = oldItemRows.length - 1; i >= 0; i--) {
      if (String(oldItemRows[i][1]) === String(body.id)) {
        cashItemRemove({ id: oldItemRows[i][0] });
      }
    }
    body.itens.forEach(function(item) {
      cashItemAdd({
...
function cashItemAdd(body) {
  ...
  if (body.tipo_item === "produto" && body.produto_id) {
    ...
    var prodRow = findRow(sheets.PROD, prodId);
    if (prodRow > 0) {
      var curEstoque = toNum(sheets.PROD.getRange(prodRow, 4).getValue());
      sheets.PROD.getRange(prodRow, 4).setValue(Math.max(0, curEstoque - toNum(body.quantidade, 1)));
    }
...
function cashItemRemove(body) {
  ...
  if (cur[2] === "produto" && cur[3]) {
    var prodRow = findRow(sheets.PROD, cur[3]);
    if (prodRow > 0) {
      var curEstoque = toNum(sheets.PROD.getRange(prodRow, 4).getValue());
      sheets.PROD.getRange(prodRow, 4).setValue(curEstoque + toNum(cur[5]));
    }
  }
...
// =====================================================
// RESTAURACAO DE SISTEMA (BACKUP)
// =====================================================
function systemRestore(body) {
  if (!body || !body.backup) return { ok: false, err: "Nenhum dado fornecido" };
  var sheets = boot();
  var backup = body.backup;
  ...
  if (backup.agendadores && Object.keys(backup.agendadores).length > 0) {
    clearSheet(sheets.PROF);
    var agArray = Array.isArray(backup.agendadores) ? backup.agendadores : Object.values(backup.agendadores);
    agArray.forEach(function(ag) {
      sheets.PROF.appendRow([
        ag.id, ag.nome || ag.name || "", ag.senha || "", ag.isAdmin ? true : false,
        ag.status || "ativo", typeof ag.permissoes === 'object' ? JSON.stringify(ag.permissoes) : ag.permissoes,
        ag.agenda_bloqueada ? true : false, ag.registrar_expediente ? true : false,
        ag.hora_inicio || "08:00", ag.hora_fim || "18:00",
        ag.almoco_inicio || "", ag.almoco_fim || "",
        ag.comissao_servicos || 0, ag.comissao_produtos || 0,
        ag.created_at || iso(), ag.updated_at || iso()
      ]);
    });
  }
  
  if (backup.clients && backup.clients.length > 0) {
    clearSheet(sheets.CLI);
    backup.clients.forEach(function(c) {
      sheets.CLI.appendRow([
        c.id, c.nome, c.telefone || c.phone || "", c.cpf || "", c.aniversario || "", c.observacoes || "",
        c.status || "ativo", c.created_at || iso(), c.updated_at || iso()
      ]);
    });
  }
  
  if (backup.services && backup.services.length > 0) {
    clearSheet(sheets.SERV);
    backup.services.forEach(function(s) {
      sheets.SERV.appendRow([
        s.id, s.nome, s.preco || 0, s.duracao || s.duracao_minutos || 30,
        s.descricao || "", s.icon || "fa-scissors", s.status || "ativo",
        s.created_at || iso(), s.updated_at || iso()
      ]);
    });
  }
  
  if (backup.products && backup.products.length > 0) {
    clearSheet(sheets.PROD);
    backup.products.forEach(function(p) {
      sheets.PROD.appendRow([
        p.id, p.nome, p.preco || 0, p.estoque || 0,
        p.descricao || "", p.icon || "fa-box", p.status || "ativo",
        p.created_at || iso(), p.updated_at || iso()
      ]);
    });
  }
  
  if (backup.transactions && backup.transactions.length > 0) {
    clearSheet(sheets.CASH);
    clearSheet(sheets.CASH_ITEMS);
    backup.transactions.forEach(function(t) {
      sheets.CASH.appendRow([
        t.id, t.type || t.tipo, t.cliente_id || t.clientPhone || "",
        t.description || t.descricao || "", t.paymentMethod || t.forma_pagamento || "",
        t.cardBrand || t.bandeira_cartao || "", t.taxa_cartao || 0,
        t.valor_original || t.value || 0, t.value || t.valor_real || 0,
        t.date || t.data || "", t.hora || "00:00", t.status || "ativo",
        t.profissional_id || "", t.createdAt || t.created_at || iso(), t.lastEditedAt || t.updated_at || iso()
      ]);
      
      if (t.items && t.items.length > 0) {
        t.items.forEach(function(i) {
          sheets.CASH_ITEMS.appendRow([
            genId(), t.id, i.type || i.tipo_item || "",
            (i.type === "product" || i.tipo_item === "produto") ? i.id : "",
            (i.type === "service" || i.tipo_item === "servico") ? i.id : "",
            i.quantity || i.quantidade || 1, i.price || i.valor_unitario || 0,
            (i.quantity || i.quantidade || 1) * (i.price || i.valor_unitario || 0),
            iso(), iso()
          ]);
        });
  ...
  return { ok: true, msg: "Backup importado e restaurado com sucesso na nuvem." };
}
`;
}
