...
async function loadTransactionsFromBackend() {
    try {
        ...
        if (result.ok && result.movimentacoes) {
            AppState.transactions = result.movimentacoes.map(t => ({
                id: t.id,
                type: t.tipo === 'entrada' ? 'income' : (t.tipo === 'saida' ? 'expense' : t.tipo),
                description: t.descricao || '',
                value: Number(t.valor_real) || Number(t.valor_original) || 0,
                originalValue: Number(t.valor_original) || 0,
                date: t.data || formatDate(new Date()),
                paymentMethod: t.forma_pagamento || '',
                cardBrand: t.bandeira_cartao || '',
                clientName: t.cliente_nome || '',
                clientPhone: t.cliente_telefone || '',
                items: t.items || [],
                editHistory: t.historico || [],
...
async function renderCashier() {
    // Carrega do backend se ainda não tiver dados
    if (!AppState.transactions || AppState.transactions.length === 0) {
        await loadTransactionsFromBackend();
    }
    // Inicializar data do filtro se nÃ£o existir
...
async function saveTransaction(index) {
    ...
    try {
        ...
        if (result.ok) {
            await loadTransactionsFromBackend();
            // Atualizar estoque de produtos se houver itens
            if (items.length > 0) {
                await updateProductStockFromItems(items, isEdit ? AppState.transactions[index]?.items : []);
            }
...
async function updateProductStockFromItems(newItems, oldItems = []) {
    // Restaurar estoque dos itens antigos (em edição)
    for (const item of oldItems) {
        if (item.type === 'product') {
            try {
                await DB.productStockAdd(item.id, item.quantity);
            } catch (e) {
    ...
    for (const item of newItems) {
        if (item.type === 'product') {
            try {
                const result = await DB.productStockRemove(item.id, item.quantity);
                if (result.ok && result.data) {
                    const newStock = result.data.estoque || 0;
                    if (newStock === 0) {
                ...
                console.error('[Estoque] Erro ao diminuir:', e);
            }
        }
    }
